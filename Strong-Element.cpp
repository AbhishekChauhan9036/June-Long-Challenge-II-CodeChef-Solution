/* Author_Of_This_Code = Chauhan Abhishek */
...
int __Hanuman__Chalisha(int ____l,int ____r,int ___PARR[],int SURRR[],int __n){
    if(____l==0){
        return SURRR[____r+1];
    }
    
    if(____r==__n-1){
        return ___PARR[____l-1];
        
    }
    
    return __gcd(___PARR[____l-1],SURRR[____r+1]);
}


void ____Ramayan(int ___PARR[],int ___ARRR[],int SURRR[],int ___n){
    ___PARR[0]=___ARRR[0];
    for(int i=1;i<___n;i++){
        ___PARR[i]=__gcd(___PARR[i-1],___ARRR[i]);
        
    }
    
    SURRR[___n-1]=___ARRR[___n-1];
    
    for(int i=___n-2;i>=0;i--){
        SURRR[i]=__gcd(SURRR[i+1],___ARRR[i]);
    }
}
/******************************************************************************************************************************************/
/***********FUNCTION TO BE ADDED AND TO BE USED****************/

void BhagwatGeeta(){
    int _,__,_1,_2,_3,_4;
	cin>>_;
	
	int ___ARRR[_],___PARR[_],SARRR[_];
	for(int i=0;i<_;i++){
	    cin>>___ARRR[i];
	}
	
    ____Ramayan(___PARR,___ARRR,SARRR,_);
    
        int ___print=0;
        for(int i=0;i<_;i++){
            if(__Hanuman__Chalisha(i,i,___PARR,SARRR,_)>1){
                ___print++;
            }
